...
// zamienia HH:MM na minuty i odwrotnie
const toMinutes = hm => { const [h,m] = hm.split(':').map(Number); return h*60 + m; };
const fmt = mins => { const h = Math.floor(mins/60), m = mins%60; return `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`; };
const DAILY_LIMIT = 8*60 + 15; // 8h 15min w minutach
...
onAuthStateChanged(auth, user => {
  ...
  if (section) {
    ...
    // ścieżka w DB: worklogs/{uid}/{YYYY-MM}
    const monthStr = dateInput.value.slice(0,7);
    const monthRef = ref(db, `worklogs/${user.uid}/${monthStr}`);
    ...
    saveBtn.addEventListener('click', () => {
      ...
      if (!d||!start||!planned||!actual) return alert('Uzupełnij wszystkie pola');
      set(ref(db, `worklogs/${user.uid}/${d}`), { date: d, start, planned, actual, report });
    });
  }
});